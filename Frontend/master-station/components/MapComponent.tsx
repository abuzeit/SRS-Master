...
const closedIcon = L.divIcon({
    className: "custom-marker",
    html: `<div class="relative flex items-center justify-center">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="h-9 w-9 text-muted-foreground" style="filter: drop-shadow(0 4px 4px rgba(0,0,0,0.5));">
                <path fill-rule="evenodd" d="M11.54 22.351l.07.04.028.016a.76.76 0 00.723 0l.028-.015.071-.041a16.975 16.975 0 001.144-.742 19.58 19.58 0 002.683-2.282c1.944-1.99 3.963-4.98 3.963-8.827a8.25 8.25 0 00-16.5 0c0 3.846 2.02 6.837 3.963 8.827a19.58 19.58 0 002.682 2.282 16.975 16.975 0 001.145.742zM12 13.5a3 3 0 100-6 3 3 0 000 6z" clip-rule="evenodd" />
            </svg>
           </div>`,
    iconSize: [36, 36],
...
function MapControls({ onZoomIn, onZoomOut, onCenter, onCurrentLocation, mapStyle, onToggleTheme }: {
    ...
    return (
        <div className="absolute top-6 right-6 flex flex-col gap-2 z-[1000]">
            <button
                onClick={onZoomIn}
                className="bg-background/85 backdrop-blur-md border border-border w-9 h-9 flex items-center justify-center rounded-md shadow-lg text-muted-foreground hover:bg-accent hover:text-foreground transition-colors"
            >
            ...
            <button
                onClick={onCenter}
                className="bg-background/85 backdrop-blur-md border border-border w-9 h-9 flex items-center justify-center rounded-md shadow-lg text-muted-foreground hover:bg-accent hover:text-foreground transition-colors mt-2"
                title="Center map"
            >
...
export default function MapComponent({ isAlarmFeedOpen = true }: { isAlarmFeedOpen?: boolean }) {
    ...
    return (
        <div className="relative w-full h-full transition-all duration-300">
                    ...
                    return (
                        ...
                        >
                            <Popup className="custom-popup">
                                <div className="bg-card/95 backdrop-blur-xl rounded-xl min-w-[240px] border border-border shadow-2xl overflow-hidden">
                                    {/* Status Header Bar */}
                                    <div className={cn(
                                        "h-1 w-full",
                                        isAlarm ? "bg-rose-500 shadow-[0_0_10px_rgba(244,63,94,0.5)]" :
                                            isMaint ? "bg-amber-500 shadow-[0_0_10px_rgba(245,158,11,0.5)]" :
                                                isClosed ? "bg-muted-foreground/60" : "bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.5)]"
                                    )} />
                                    ...
                                    <div className="p-3">
                                        ...
                                        <div className="flex justify-between items-start mb-2.5">
                                            <div>
                                                <div className="flex items-center gap-1.5">
                                                    <h3 className="font-black text-foreground text-base tracking-tighter">{station.id}</h3>
                                                    <span className={cn(
                                                        ...
                                                        isAlarm ? "text-rose-400 border-rose-500/30 bg-rose-500/10" :
                                                            isMaint ? "text-amber-400 border-amber-500/30 bg-amber-500/10" :
                                                                isClosed ? "text-muted-foreground border-border bg-muted/50" :
                                                                    "text-emerald-400 border-emerald-500/30 bg-emerald-500/10"
                                                    )}>
                                                        {station.status}
                                                    </span>
                                                </div>
                                                <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-wide mt-0.5">{station.name}</p>
                                            </div>
                                            <div className="bg-muted/50 p-1.5 rounded-lg border border-border/50">
                                                <Activity className={cn(
                                                    "w-3.5 h-3.5",
                                                    isAlarm ? "text-rose-500" : isMaint ? "text-amber-500" : isClosed ? "text-muted-foreground" : "text-emerald-500"
                                                )} />
                                            </div>
                                        </div>
                                        ...
                                        {isAlarm && station.alarmDetail && (
                                            <div className="mb-2.5 bg-rose-500/10 border border-rose-500/20 rounded-lg p-2 flex items-center gap-2">
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    <p className="text-xs text-rose-500 font-medium leading-none truncate">{station.alarmDetail}</p>
                                                </div>
                                            </div>
                                        )}
                                        ...
                                        <div className="grid grid-cols-2 gap-1.5 mb-3">
                                            <div className="bg-muted/50 border border-border/50 rounded-lg px-2 py-1 flex items-center justify-between">
                                                <div className="flex items-center gap-1 text-muted-foreground">
                                                    <Droplets className="w-2.5 h-2.5" />
                                                    <span className="text-[8px] font-black uppercase tracking-tighter">Average pH</span>
                                                </div>
                                                <p className="text-[11px] font-black text-foreground font-mono">{station.ph || "--"}</p>
                                            </div>
                                            <div className="bg-muted/50 border border-border/50 rounded-lg px-2 py-1 flex items-center justify-between">
                                                <div className="flex items-center gap-1 text-muted-foreground">
                                                ...
                                                <p className="text-[11px] font-black text-foreground font-mono">{station.conductivity || "--"}</p>
                                            </div>
                                            <div className="bg-muted/50 border border-border/50 rounded-lg px-2 py-1 flex items-center justify-between">
                                                <div className="flex items-center gap-1 text-muted-foreground">
                                                ...
                                                <p className="text-[11px] font-black text-foreground font-mono">{station.flowRate || "0"}</p>
                                            </div>
                                            <div className="bg-muted/50 border border-border/50 rounded-lg px-2 py-1 flex items-center justify-between">
                                                <div className="flex items-center gap-1 text-muted-foreground">
                                                ...
                                                <p className="text-[11px] font-black text-foreground font-mono">{station.uptime}</p>
                                            </div>
                                        </div>
                                        ...
                                        <Link
                                            ...
                                            className="flex items-center justify-center gap-1.5 w-full py-2 bg-accent hover:bg-accent/80 text-foreground text-[11px] font-bold rounded-lg transition-all border border-border/50 group"
                                        >
                                            View Details
            ...
            <div className="absolute top-6 left-6 w-80 bg-background/85 backdrop-blur-md rounded-xl shadow-2xl z-[1000] flex flex-col max-h-[calc(100%-3rem)] overflow-hidden transition-all duration-300 border border-border">
                <Accordion defaultValue={["ranking"]} className="w-full">
                    <AccordionItem value="ranking" className="border-b-0">
                        <AccordionTrigger className="px-4 py-4 hover:no-underline hover:bg-accent/50 border-b border-border bg-muted/40 data-[state=open]:border-b">
                        ...
                        <AccordionContent className="p-0 border-none m-0 overflow-y-auto">
                            <div className="p-3 space-y-2.5">
                                ...
                                <div className="bg-gradient-to-r from-blue-500/10 to-transparent rounded-lg p-2.5 border border-blue-500/20">
                                    <div className="text-[10px] text-blue-500 font-bold mb-1 uppercase tracking-wide">Total Network Discharge</div>
                                    <div className="text-2xl font-bold text-foreground font-mono tracking-tight">15,482 <span className="text-sm font-normal text-muted-foreground">m³</span></div>
                                ...
                                <Accordion defaultValue={["top"]} className="w-full">
                                    <AccordionItem value="top" className="border-border">
                                        ...
                                        <AccordionContent className="pt-1 pb-2">
                                            <div className="space-y-1.5">
                                                <div className="flex items-center justify-between p-2 bg-accent/40 rounded-md border border-border hover:border-blue-500/50 hover:bg-accent/60 transition-all cursor-pointer group">
                                                    <div className="flex items-center gap-3">
                                                        <div className="font-bold text-blue-500/60 text-xs font-mono group-hover:text-blue-500">01</div>
                                                        <div>
                                                            <div className="text-xs font-medium text-foreground leading-tight">SRS-02</div>
                                                            <div className="text-[10px] text-muted-foreground leading-tight">Mussafah</div>
                                                        </div>
                                                    </div>
                                                    <div className="text-right">
                                                        <div className="text-xs font-mono font-bold text-foreground">1,240 m³</div>
                                                    </div>
                                                </div>
                                                ...
                                                <div className="flex items-center justify-between p-2 bg-accent/40 rounded-md border border-border hover:border-blue-500/50 hover:bg-accent/60 transition-all cursor-pointer group">
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <div>
                                                            <div className="text-xs font-medium text-foreground leading-tight">SRS-05</div>
                                                            <div className="text-[10px] text-muted-foreground leading-tight">Al Ain Central</div>
                                                        </div>
                                                    </div>
                                                    <div className="text-right">
                                                        <div className="text-xs font-mono font-bold text-foreground">980 m³</div>
                                                    </div>
                                                </div>
                                                ...
                                                <div className="flex items-center justify-between p-2 bg-accent/40 rounded-md border border-border hover:border-blue-500/50 hover:bg-accent/60 transition-all cursor-pointer group">
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <div>
                                                            <div className="text-xs font-medium text-foreground leading-tight">SRS-11</div>
                                                            <div className="text-[10px] text-muted-foreground leading-tight">Shahama</div>
                                                        </div>
                                                    </div>
                                                    <div className="text-right">
                                                        <div className="text-xs font-mono font-bold text-foreground">855 m³</div>
                                                    </div>
                                                </div>
                                            </div>
                                        </AccordionContent>
                                    ...
                                    <AccordionItem value="critical" className="border-b-0">
                                        ...
                                        <AccordionContent className="pt-1 pb-2">
                                            <div className="space-y-1.5">
                                                <div className="flex items-center justify-between p-2 bg-rose-500/10 rounded-md border border-rose-500/20 cursor-pointer hover:bg-rose-500/20 transition-colors">
                                                    <div className="flex items-center gap-3">
                                                        <div className="w-1.5 h-1.5 rounded-full bg-rose-500 animate-pulse"></div>
                                                        <div>
                                                            <div className="text-xs font-medium text-foreground leading-tight">SRS-14</div>
                                                            <div className="text-[10px] text-rose-500 font-bold leading-tight uppercase">AC Failure</div>
                                                        </div>
                                                    </div>
                                                    <div className="text-right">
                                                        <div className="text-xs font-mono font-bold text-muted-foreground">0 m³</div>
                                                    </div>
                                                </div>
                                                ...
                                                <div className="flex items-center justify-between p-2 bg-rose-500/10 rounded-md border border-rose-500/20 cursor-pointer hover:bg-rose-500/20 transition-colors">
                                                    <div className="flex items-center gap-3">
                                                        <div className="w-1.5 h-1.5 rounded-full bg-rose-500 animate-pulse"></div>
                                                        <div>
                                                            <div className="text-xs font-medium text-foreground leading-tight">SRS-22</div>
                                                            <div className="text-[10px] text-rose-500 font-bold leading-tight uppercase">High pH</div>
                                                        </div>
                                                    </div>
                                                    <div className="text-right">
                                                        <div className="text-xs font-mono font-bold text-muted-foreground">12 m³</div>
                                                    </div>
                                                </div>
                                            </div>
                                        </AccordionContent>
            ...
            <div className="absolute bottom-4 right-4 bg-background/85 backdrop-blur-md rounded-lg p-3 shadow-2xl z-[1000] border border-border">
                ...
                <div className="flex flex-col gap-2.5">
                    ...
                    >
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-foreground text-xs font-medium">Operational</span>
                        </div>
                        <span className="text-[10px] font-mono text-muted-foreground">({statusCounts['Operational'] || 0})</span>
                    </button>
                    <button
                    ...
                    >
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-foreground text-xs font-medium">Alarm</span>
                        </div>
                        <span className="text-[10px] font-mono text-muted-foreground">({statusCounts['Alarm'] || 0})</span>
                    </button>
                    <button
                    ...
                    >
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-foreground text-xs font-medium">Maint.</span>
                        </div>
                        <span className="text-[10px] font-mono text-muted-foreground">({statusCounts['Maint.'] || 0})</span>
                    </button>
                    <button
                    ...
                    >
                        <div className="flex items-center gap-2">
                            <span className="w-2.5 h-2.5 rounded-full bg-muted-foreground/60"></span>
                            <span className="text-foreground text-xs font-medium">Closed</span>
                        </div>
                        <span className="text-[10px] font-mono text-muted-foreground">({statusCounts['Closed'] || 0})</span>
                    </button>
                </div>
            </div>
        </div>
